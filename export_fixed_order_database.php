<?php
/**
 * Fixed Order Database Export Script
 * Exports database with proper table creation order to avoid foreign key errors
 */
...
// Output file
$outputFile = 'manuela_fixed_order_export.sql';

echo "🚀 Starting FIXED ORDER database export...\n";
echo "📊 This will export EVERYTHING in the correct order!\n\n";

// Start SQL file
$sql = "-- =============================================\n";
$sql .= "-- ManuelCode FIXED ORDER Database Export\n";
$sql .= "-- Generated: " . date('Y-m-d H:i:s') . "\n";
$sql .= "-- Database: $dbname\n";
$sql .= "-- Tables created in correct order - no foreign key errors!\n";
...
$sql .= "-- Temporarily disable foreign key checks\n";
...
// Define table creation order (parent tables first, then child tables)
$tableOrder = [
...
// Create a map of tables to their order
$tableOrderMap = array_flip($tableOrder);

// Sort tables by their defined order
$sortedTables = [];
...
foreach ($tables as $tableInfo) {
    if (!in_array($tableInfo['name'], $tableOrder)) {
        $sortedTables[] = $tableInfo;
    }
}

echo "📤 Exporting tables in correct order...\n";

// Export each table/view in the sorted order
foreach ($sortedTables as $tableInfo) {
...
if (file_put_contents($outputFile, $sql) !== false) {
    echo "\n🎉 FIXED ORDER Database export finished successfully!\n";
    ...
    echo "🔗 Tables created in correct order - no foreign key errors!\n";
    echo "🐬 MariaDB compatible - no syntax errors\n";
} else {
