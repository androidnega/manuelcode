<?php
/**
 * Complete Database Export Script - MariaDB Compatible
...
// Output file
$outputFile = 'manuela_complete_export_fixed.sql';

echo "🚀 Starting COMPLETE database export (MariaDB Compatible)...\n";
echo "📊 This will export EVERYTHING - no data loss!\n\n";

// Start SQL file
$sql = "-- =============================================\n";
$sql .= "-- ManuelCode COMPLETE Database Export\n";
$sql .= "-- Generated: " . date('Y-m-d H:i:s') . "\n";
$sql .= "-- Database: $dbname\n";
$sql .= "-- MariaDB Compatible - No DEFINER issues\n";
...
foreach ($tables as $tableInfo) {
    ...
    try {
        if ($tableType === 'BASE TABLE') {
            ...
            // Export ALL table data using simple SELECT (no LIMIT/OFFSET issues)
            ...
            if ($totalRows > 0) {
                ...
                // Export data in simple batches without complex LIMIT/OFFSET
                $batchSize = 100;
                $processed = 0;
                
                while ($processed < $totalRows) {
                    // Use simple LIMIT without OFFSET for MariaDB compatibility
...
if (file_put_contents($outputFile, $sql) !== false) {
    echo "\n🎉 COMPLETE Database export finished successfully!\n";
