...
// Output file
$outputFile = 'manuela_complete_export.sql';
    ...
    try {
        if ($tableType === 'BASE TABLE') {
            ...
            if ($totalRows > 0) {
                ...
                // Export data in optimal batches
                $batchSize = 50; // Smaller batches for better compatibility
                $offset = 0;
                
                while ($offset < $totalRows) {
                    $stmt = $pdo->prepare("SELECT * FROM `$tableName` LIMIT ? OFFSET ?");
